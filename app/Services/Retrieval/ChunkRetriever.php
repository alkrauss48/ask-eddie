...
/**
 * Hybrid retrieval: a dense channel, a lexical channel, fused and reranked.
 *
 ...
 * hatch for exactly this, so the tool below wraps this class rather than
 * forking anything.
 */
class ChunkRetriever
{
    ...
    public function retrieve(string $query, ?int $limit = null): Collection
    {
        ...
        $limit ??= (int) config('books.retrieval.limit');
        ...
        $fused = $this->fusion->fuse(
            $channels,
            [
                self::DENSE => (float) config('books.retrieval.weights.dense'),
                self::LEXICAL => (float) config('books.retrieval.weights.lexical'),
            ],
            (int) config('books.retrieval.rrf_k'),
        );
    ...
    {
        $candidates = (int) config('books.retrieval.dense_candidates');

        // HNSW visits ef_search nodes and returns what it finds among them, so
        // an ef_search below the candidate count silently returns a short list
        // -- which looks like a thin corpus rather than a mis-set knob. "local"
        // scopes it to this transaction-less statement's session use only.
        DB::statement('set local hnsw.ef_search = '.max(
            $candidates,
            (int) config('books.retrieval.ef_search'),
        ));
        ...
        return BookChunk::query()
            ...
            ->whereVectorSimilarTo(
                'embedding',
                $this->embed($query),
                (float) config('books.retrieval.min_similarity'),
            )
            ->limit($candidates)
            ->pluck('id')
            ->all();
    ...
    {
        $config = (string) config('books.retrieval.text_search_config');
        ...
        return BookChunk::query()
            ...
            ->limit((int) config('books.retrieval.lexical_candidates'))
            ->pluck('id')
            ->all();
    }
    ...
    public function embed(string $query): array
    {
        $response = Embeddings::for([$query])
            ...
            ->generate(
                (string) config('books.embedding.provider'),
